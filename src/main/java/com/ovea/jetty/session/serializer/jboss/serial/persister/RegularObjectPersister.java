       ...
       for (int fieldNR=0;fieldNR<fields.length;fieldNR++)
       {
           ...
           else
           {
               ...
               if (isDebug)
               {
                   if (value==null)
                   {
                       log.debug("writeSlotWithDefaultMethod slot=" + slot.getSlotClass().getName() + " objectField " + fields[fieldNR].getFieldName() + " with object=NULL");
                   }
                   else
                   {
                       log.debug("writeSlotWithDefaultMethod slot=" + slot.getSlotClass().getName() + " objectField " + fields[fieldNR].getFieldName() + " with object=" + value.getClass().getName());
                   }
               }
           }
       }
